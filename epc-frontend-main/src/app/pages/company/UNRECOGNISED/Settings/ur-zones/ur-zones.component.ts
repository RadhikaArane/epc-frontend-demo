...
import { Zones } from '../../../../../shared/models/unrecognisedInvoice-models/settings';

@Component({
  selector: 'app-ur-zones',
  standalone: true,
  imports: [CommonModule, BreadcrumbsComponent],
  templateUrl: './ur-zones.component.html',
  styleUrl: './ur-zones.component.scss'
})
export class UrZonesComponent implements OnInit, OnDestroy {
  ...
  private loadZones(): void {
    ...
    console.log('Loading Zones with params:', params);

    this.urSettingSrv._getZones(params)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (res: any) => {
          console.log("Zones response:", res);

          this.allZones.set(res.Items || []);
        ...
        error: (err: any) => {
          ...
          this.toastSrv.showToast(
            err?.message || 'Error occurred while loading Zones.',
            'error'
          );
          this.isLoading.set(false);
          this.allZones.set([]);
