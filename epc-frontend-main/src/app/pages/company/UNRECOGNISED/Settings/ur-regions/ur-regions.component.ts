...
import { Regions } from '../../../../../shared/models/unrecognisedInvoice-models/settings';


@Component({
  selector: 'app-ur-regions',
  standalone: true,
  imports: [CommonModule, BreadcrumbsComponent],
  templateUrl: './ur-regions.component.html',
  styleUrl: './ur-regions.component.scss'
})
export class UrRegionsComponent implements OnInit, OnDestroy{
  ...
  authSrv = inject(AuthService);
    ...
    private loadRegions(): void {
      ...
      console.log('Loading Regions with params:', params);
  
      this.urSettingSrv._getRegions(params)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: (res: any) => {
            console.log("Regions response:", res);
  
            this.allRegions.set(res.Items || []);
          ...
          error: (err: any) => {
            ...
            this.toastSrv.showToast(
              err?.message || 'Error occurred while loading Regions.',
              'error'
            );
            this.isLoading.set(false);
            this.allRegions.set([]);
