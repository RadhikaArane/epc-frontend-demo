...
import { Areas } from '../../../../../shared/models/unrecognisedInvoice-models/settings';

@Component({
  selector: 'app-ur-areas',
  standalone: true,
  imports: [CommonModule,BreadcrumbsComponent],
  templateUrl: './ur-areas.component.html',
  styleUrl: './ur-areas.component.scss'
})
export class UrAreasComponent implements OnInit, OnDestroy{
  ...
  private loadAreas(): void {
    ...
    console.log('Loading Areas with params:', params);

    this.urSettingSrv._getAreas(params)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (res: any) => {
          console.log("Access response:", res);

          this.allAreas.set(res.Items || []);
        ...
        error: (err: any) => {
          ...
          this.toastSrv.showToast(
            err?.message || 'Error occurred while loading Areas.',
            'error'
          );
          this.isLoading.set(false);
          this.allAreas.set([]);
