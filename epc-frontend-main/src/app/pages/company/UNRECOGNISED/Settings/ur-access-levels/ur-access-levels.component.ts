...
import { AccessLevels } from '../../../../../shared/models/unrecognisedInvoice-models/settings';

@Component({
  selector: 'app-ur-access-levels',
  standalone: true,
  imports: [CommonModule,BreadcrumbsComponent],
  templateUrl: './ur-access-levels.component.html',
  styleUrl: './ur-access-levels.component.scss'
})
export class UrAccessLevelsComponent implements OnInit, OnDestroy{
  ...
  authSrv = inject(AuthService);
    ...
    allAccessLevels = signal<AccessLevels[]>([]); 
    ...
    private loadAccess(): void {
      ...
      console.log('Loading Access Levels with params:', params);
  
      this.urSettingSrv._getAccessLevels(params)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: (res: any) => {
            console.log("Access response:", res);
  
            this.allAccessLevels.set(res.Items || []);
          ...
          error: (err: any) => {
            console.error("Error loading Access Levels:", err);
            this.toastSrv.showToast(
              err?.message || 'Error occurred while loading Access Levels.',
              'error'
            );
            this.isLoading.set(false);
            this.allAccessLevels.set([]);
