...
import { Roles } from '../../../../../shared/models/unrecognisedInvoice-models/settings';

@Component({
  selector: 'app-ur-roles',
  standalone: true,
  imports: [CommonModule, BreadcrumbsComponent],
  templateUrl: './ur-roles.component.html',
  styleUrl: './ur-roles.component.scss'
})
export class UrRolesComponent implements OnInit, OnDestroy{
  ...
  private loadRoles(): void {
    ...
    this.urSettingSrv._getRoles(params)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (res: any) => {
          console.log("Roles response:", res);

          this.allRoles.set(res.Items || []);
        ...
        error: (err: any) => {
          ...
          this.toastSrv.showToast(
            err?.message || 'Error occurred while loading Roles.',
            'error'
          );
          this.isLoading.set(false);
          this.allRoles.set([]);
