...
// Models
import { PanIndiaSummaryBucketItemResponse, PanIndiaSummaryBucketResponse, } from '../../../../../../shared/models/unrecognisedInvoice-models/Inventory';
...
interface ColumnConfig {
  key: string; // Exact API key e.g. "Inv. Ref."
  label: string; // Display label
  type: 'text' | 'currency' | 'date';
  sticky: boolean; // Frozen column?
  collapsible: boolean;
...
@Component({
  selector: 'app-ur-pan-india-bucket-summary',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  providers: [DatePipe],
  templateUrl: './ur-pan-india-bucket-summary.component.html',
  styleUrl: './ur-pan-india-bucket-summary.component.scss',
})

export class UrPanIndiaBucketSummaryComponent implements OnInit, OnDestroy {
  ...
  // Table config — built dynamically from API response
  ...
  // Known column type mappings (keys that need special treatment)
    ...
    // 2. Add custom labels and shrink the widths for Sales Value and State
    'Sales Value': { type: 'currency', minWidth: 90, maxWidth: 90, label: 'Sales\nValue', wrap: true },
    'State': { type: 'text', minWidth: 90, maxWidth: 90, wrap: true }, 
  ...
  // Columns to skip from the table (if needed)
  ...
  // Computed: sticky (frozen) columns
  ...
  this.activatedRoute.queryParams
    ...
    .subscribe((params) => {
      ...
      try {
        ...
        this.selectedDate.set(data.date); 
  ...
  loadData(selectedDate?: string): void {
    ...
    this.urInventorySrv
      ._getInventoryPanIndiaBucketSummary(params)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (res) => {
          ...
          // Build columns from first item's keys (only once or when columns are empty)
  ...
  /**
   * Build column config dynamically from the API response keys
   */
...
for (const key of keys) {
  ...
  if (key === 'Sales Value') afterSalesValue = true;

  // default values from map
  let minWidth = typeInfo?.minWidth;
  let maxWidth = typeInfo?.maxWidth;
  let wrap = typeInfo?.wrap ?? false;

  // ✅ Sales Value TARVATHA columns: fixed 150 + wrap
  if (afterSalesValue && key !== 'Sales Value' && !typeInfo?.minWidth) {
        minWidth = 150;
        maxWidth = 150;
        wrap = true;
      }
    ...
    // Ensure sticky columns come first
  ...
  getTotalVisibleColumns(): number {
    ...
    return (
      1 + this.columns().filter((c) => !this.isColumnCollapsed(c.key)).length
    );
  }
  ...
  navigateBack(): void {
    this.router.navigate(['/unrecogmisedInventoryComponent'], {
      queryParams: { tab: this.sourceTab() },
    });
  }
  ...
  downloadExcel(): void {
    ...
    this.urInventorySrv
      ._excelInventoryPanIndiaBucketSummary({
        State: this.selectedState(),
        AreaName: '', 
        Ageing: this.selectedAgeing(),
        StageName: this.selectedStatus(), 
      ...
      .subscribe({
        next: (blob) => {
          ...
          a.href = url;
          a.download = `Inventory_PanIndia_${this.selectedState() || 'All'}.xlsx`;
          a.click();
