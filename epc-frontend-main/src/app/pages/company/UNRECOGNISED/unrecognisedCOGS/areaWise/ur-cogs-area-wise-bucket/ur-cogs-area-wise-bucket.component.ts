...
@Component({
  selector: 'app-ur-cogs-area-wise-bucket',
  standalone: true,
  imports: [CommonModule, FormsModule, BreadcrumbsComponent, AddRemarksComponent],
  templateUrl: './ur-cogs-area-wise-bucket.component.html',
  styleUrl: './ur-cogs-area-wise-bucket.component.scss'
})
export class UrCogsAreaWiseBucketComponent implements OnInit, OnDestroy {
  ...
  private readonly columnTypeMap: Record<string, { type: 'text' | 'currency' | 'date'; sticky?: boolean; width?: number; label?: string }> = {
    ...
    'AreaName': { type: 'text', width: 120, label: 'Area Name' },
    'DealerName': { type: 'text', width: 150, label: 'Dealer Name' },
    'Stock Value': { type: 'currency', width: 110, label: 'Stock\nValue' },
    'Sales Value': { type: 'currency', width: 110, label: 'Sales\nValue' },
    'Value Unrestricted': { type: 'currency', width: 110, label: 'Value\nUnrestricted' },
    'Billing Date': { type: 'date' },
    'Days': { type: 'text', width: 90 },
  };

  private readonly skipColumns = new Set<string>(['Id']);
  ...
  loadData(selectedDate?: string): void {
    ...
    this.urCOGSSrv._getCOGSAreaWiseBucketSummary(params)
      ...
      .subscribe({
        next: (res) => {
          ...
          // Build columns dynamically based on the first object keys, just like state-wise
  ...
  getRowId(item: Record<string, any>): string {
    return String(item['Reg No'] || item['Billing Document'] || item['Billing No'] || item['Id'] || '');
  }
  ...
  downloadExcel(): void {
    ...
    this.urCOGSSrv._excelCOGSAreaWiseBucketSummary({ AreaName: this.selectedArea(), Ageing: this.selectedAgeing() })
      .subscribe({
        next: (blob) => {
          const url = window.URL.createObjectURL(blob);
          const a = document.createElement('a');
          a.href = url; a.download = `AreaWiseCOGS_${this.selectedArea()}.xlsx`; a.click();
