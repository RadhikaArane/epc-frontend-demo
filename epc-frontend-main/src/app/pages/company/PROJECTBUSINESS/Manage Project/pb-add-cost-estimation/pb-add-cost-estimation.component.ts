...
export class PbAddCostEstimationComponent implements OnInit, OnDestroy {
  ...
  onDocumentClick(event: MouseEvent): void {
    ...
    if (!target.closest('.vendor-dropdown-container') &&
      ...
      !target.closest('.sub-head-vendor-dropdown-container')) {  // ✅ NEW

      const hasOpen = this.costEstimationRows().some(r =>
        r.showVendorDropdown ||
        r.showSubHeadDropdown ||
        r.subHeads.some(sh => sh.showVendorDropdown)  // ✅ NEW
      );
      if (hasOpen) {
        this.costEstimationRows.set(
          this.costEstimationRows().map(row => ({
            ...
            subHeads: row.subHeads.map(sh => ({ ...sh, showVendorDropdown: false }))  // ✅ NEW
          }))
        );
      }
    }
  ...
  // ==================== Sub Head Vendor Selection ====================

  toggleSubHeadVendorDropdown(rowId: string, subHeadRowId: string): void {
  this.costEstimationRows.set(
    this.costEstimationRows().map(row => ({
      ...row,
      showVendorDropdown: false,      // ✅ close parent vendor dropdown
      showSubHeadDropdown: false,      // ✅ close sub head add dropdown
      subHeads: row.subHeads.map(sh => ({
        ...sh,
        showVendorDropdown:
          row.id === rowId && sh.id === subHeadRowId
            ? !sh.showVendorDropdown
            : false
      }))
    }))
  );
  ...
  toggleSubHeadVendorSelection(rowId: string, subHeadRowId: string, vendor: Vendor): void {
    const row = this.costEstimationRows().find(r => r.id === rowId);
    const subHead = row?.subHeads.find(sh => sh.id === subHeadRowId);
    if (!subHead) return;

    const exists = subHead.selectedVendors.findIndex(v => v.VendorId === vendor.VendorId);
    let updatedVendors: Vendor[];
    let updatedAttachments: VendorAttachment[];

    if (exists > -1) {
      updatedVendors = subHead.selectedVendors.filter((_, i) => i !== exists);
      updatedAttachments = subHead.vendorAttachments.filter(va => va.vendorId !== vendor.VendorId);
      this.toastSrv.showToast(`${vendor.Name} removed`, 'warning');
    } else {
      updatedVendors = [...subHead.selectedVendors, vendor];
      updatedAttachments = [
        ...subHead.vendorAttachments,
        { vendorId: vendor.VendorId, vendorName: vendor.Name, file: null, files: [] as File[], assignedAmount: '' }
      ];
      this.toastSrv.showToast(`${vendor.Name} added`, 'success');
    }
      ...
      this.costEstimationRows().map(row =>
        row.id === rowId
          ? {
            ...
            subHeads: row.subHeads.map(sh =>
              sh.id === subHeadRowId
                ? { ...sh, selectedVendors: updatedVendors, vendorAttachments: updatedAttachments }
                : sh
            )
          }
          : row
  ...
  isSubHeadVendorSelected(rowId: string, subHeadRowId: string, vendorId: string): boolean {
    const row = this.costEstimationRows().find(r => r.id === rowId);
    const subHead = row?.subHeads.find(sh => sh.id === subHeadRowId);
    return subHead?.selectedVendors.some(v => v.VendorId === vendorId) || false;
  }

  getSubHeadVendorAmount(rowId: string, subHeadRowId: string, vendorId: string): string {
    const row = this.costEstimationRows().find(r => r.id === rowId);
    const subHead = row?.subHeads.find(sh => sh.id === subHeadRowId);
    return subHead?.vendorAttachments.find(va => va.vendorId === vendorId)?.assignedAmount || '';
  }

  updateSubHeadVendorAmount(rowId: string, subHeadRowId: string, vendorId: string, amount: string): void {
    this.costEstimationRows.set(
      this.costEstimationRows().map(row =>
        row.id === rowId
          ? {
            ...
            subHeads: row.subHeads.map(sh =>
              sh.id === subHeadRowId
                ? {
                  ...sh,
                  vendorAttachments: sh.vendorAttachments.map(va =>
                    va.vendorId === vendorId ? { ...va, assignedAmount: amount } : va
                  )
                }
                : sh
  ...
  onSubHeadVendorFileSelect(rowId: string, subHeadRowId: string, vendorId: string, event: Event): void {
    const input = event.target as HTMLInputElement;
  ...
  removeSubHeadVendorFile(rowId: string, subHeadRowId: string, vendorId: string, fileIndex: number): void {
    this.costEstimationRows.set(
      this.costEstimationRows().map(row =>
        row.id === rowId
          ? {
            ...
            subHeads: row.subHeads.map(sh =>
              sh.id === subHeadRowId
                ? {
                  ...
                  vendorAttachments: sh.vendorAttachments.map(va =>
                    va.vendorId === vendorId
                      ? { ...va, files: va.files.filter((_, i) => i !== fileIndex) }
                      : va
                  )
                }
                : sh
  ...
  private proceedToVendors(): void {
    const hasVendors = this.costEstimationRows().some(
      row =>
        row.selectedVendors.length > 0 ||
        row.subHeads.some(sh => sh.selectedVendors.length > 0)  // ✅ NEW
    );
    if (!hasVendors) {
  ...
  private assignAllVendors(costHeadMapping: { [key: string]: string }): void {
    ...
    // ✅ Lookup map: `${projectCostHeadId}:${vendorId}` → subCostHeadId | undefined
    // Used later in uploadAllVendorAttachments to find files at correct level
    const vendorSubHeadMap = new Map<string, string | undefined>();

    for (const row of rows) {
      const projectCostHeadId = costHeadMapping[row.costHeadId];
      if (!projectCostHeadId) continue;

      const vendorAssignmentsArray: any[] = [];

      // ✅ Parent-level vendors → no subCostHeadId
      for (const vendor of row.selectedVendors) {
        const attachment = row.vendorAttachments.find(va => va.vendorId === vendor.VendorId);
        const amount = parseFloat(attachment?.assignedAmount || '0');
        if (amount <= 0) continue;

        vendorAssignmentsArray.push({ vendorId: vendor.VendorId, assignedAmount: amount });
        vendorSubHeadMap.set(`${projectCostHeadId}:${vendor.VendorId}`, undefined);
      }

      // ✅ Sub head level vendors → include subCostHeadId
      for (const subHead of row.subHeads) {
        for (const vendor of subHead.selectedVendors) {
          const attachment = subHead.vendorAttachments.find(va => va.vendorId === vendor.VendorId);
          const amount = parseFloat(attachment?.assignedAmount || '0');
          if (amount <= 0) continue;

          vendorAssignmentsArray.push({
            vendorId: vendor.VendorId,
            assignedAmount: amount,
            subCostHeadId: subHead.subHeadId  // ✅ key addition
          });
          vendorSubHeadMap.set(`${projectCostHeadId}:${vendor.VendorId}`, subHead.subHeadId);
        }
      }
      ...
      .subscribe({
        next: (response: any) => {
          const hasAttachments =
            this.costEstimationRows().some(row =>
              row.vendorAttachments.some(va => va.files.length > 0) ||
              row.subHeads.some(sh => sh.vendorAttachments.some(va => va.files.length > 0))  // ✅ NEW
            );
          if (!hasAttachments) {
            this.toastSrv.showToast('Cost estimation saved successfully!', 'success');
            this.completeSubmission(true);
          } else {
            // ✅ Pass lookup map to attachment upload phase
            this.uploadAllVendorAttachments(response, costHeadMapping, vendorSubHeadMap);
          }
        },
        error: (error) => {
  ...
  private async uploadAllVendorAttachments(
    ...
    vendorSubHeadMap: Map<string, string | undefined>   // ✅ NEW param
  ): Promise<void> {
    ...
    try {
      for (const result of results) {
        ...
        for (const assignedVendor of assignedVendors) {
          const {
            ProjectCostHeadId: plainProjectCostHeadId,
            ProjectCostHeadVendorId: projectCostHeadVendorId,
            VendorId: vendorId
          } = assignedVendor;
          if (!plainProjectCostHeadId || !projectCostHeadVendorId || !vendorId) continue;

          // ✅ Look up whether this vendor was assigned at sub head level
          const subCostHeadId = vendorSubHeadMap.get(`${plainProjectCostHeadId}:${vendorId}`);
          ...
          if (subCostHeadId) {
            // ✅ Files live on the sub head's vendorAttachments
            const subHead = row.subHeads.find(sh => sh.subHeadId === subCostHeadId);
            const vendorAttachment = subHead?.vendorAttachments.find(va => va.vendorId === vendorId);
            files = vendorAttachment?.files || [];
          } else {
            // ✅ Files live on the parent row's vendorAttachments
          ...
          try {
            await this.uploadSingleVendorAttachment(projectCostHeadVendorId, files, subCostHeadId);
          } catch {
            failedUploads++;
          }
        }
  ...
  private uploadSingleVendorAttachment(
    projectCostHeadVendorId: string,
    files: File[],
    subCostHeadId?: string   // ✅ NEW optional param
  ): Promise<any> {
    return new Promise((resolve, reject) => {
      const formData = new FormData();
      formData.append('Items[0].ProjectCostHeadVendorId', projectCostHeadVendorId);

      // ✅ Include subCostHeadId only when vendor belongs to a sub head
      if (subCostHeadId) {
        formData.append('Items[0].SubCostHeadId', subCostHeadId);
      }
