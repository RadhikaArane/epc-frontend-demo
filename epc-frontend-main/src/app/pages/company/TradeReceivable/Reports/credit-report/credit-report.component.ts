...
import { CreditSummary, StateAccordionDataCredit, CreditDetailResponse } from '../../../../../shared/models/tradeReceivable-models/workingFiles';
...
export class CreditReportComponent {
  ...
  summaryData = signal<CreditSummary[]>([]);
  stateAccordions = signal<Map<string, StateAccordionDataCredit>>(new Map());

  // Dynamic columns from API
  detailColumns = signal<string[]>([]);

  // Currency columns
  private readonly currencyKeys = new Set(['Amt', 'TotalCreditAmount']);
  ...
  ngOnInit() {
    if (this.selectedMonth() && this.selectedYear()) this.loadSummary();
  }
  ...
  loadSummary() {
    ...
    this.srv._getCreditSummary(this.selectedMonth(), this.selectedYear())
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (res: CreditSummary[]) => {
          this.summaryData.set(res || []);
          this.isLoadingSummary.set(false);
        },
        error: (err: any) => {
          this.toastSrv.showToast(err?.message || 'Error loading credit summary', 'error');
          this.isLoadingSummary.set(false);
  ...
  loadStateData(state: string) {
    ...
    this.srv._getCredirByStatePaged(state, this.selectedMonth(), this.selectedYear(), data.currentPage, data.pageSize)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (res: CreditDetailResponse) => {
  ...
  getStateData(state: string): StateAccordionDataCredit | undefined {
